...
class OnlineGameActivity : AppCompatActivity() {
    ...
    private fun removeCode(){
        if(isCodeMaker){
            FirebaseDatabase.getInstance().reference.child("codes").child(gameKey).removeValue()
    ...
    private fun simpleGame(gameName : String) {
        ...
        if(isCodeMaker){
            player1Name.text = "You"
            player2Name.text = "Your Friend"
        }
        else
        {
            player2Name.text = "You"
            player1Name.text = "Your Friend"
        }
        val myPlayer: Player = if(isCodeMaker) player1 else player2
        var startX: Int = 0
        var startY: Int = 0
        var endX: Int = 0
        var endY: Int = 0
        ...
        FirebaseDatabase.getInstance().reference.child("data").child(code)
            .addChildEventListener(object : ChildEventListener {
                override fun onChildAdded(snapshot: DataSnapshot, previousChildName: String?) {
                    val data =
                        ((snapshot.value.toString()).substringBefore("}")).substringAfter("=")
                    val isWhitePlayer: Boolean = data.substringBefore(" ") == "true"
                    val player = if(isWhitePlayer) player1 else
                        player2
                    val coordinates = data.substringAfter(" ")
                    val startCoordinates = coordinates.substringBefore(" ").toInt()
                    val endCoordinates = coordinates.substringAfter(" ").toInt()
                    val newStartX = startCoordinates / 10
                    val newStartY = startCoordinates % 10
                    val newEndX = endCoordinates / 10
                    val newEndY = endCoordinates % 10
                    if (game.playerMove(player, newStartX, newStartY, newEndX, newEndY)) {
                        updateBoard(newStartX, newStartY, newEndX, newEndY)
                        startTimer(timerCounter, game, gameName)
                        openWinDialog(game)
                    }
            ...
            row0col0.setOnClickListener {
                ...
                endX = 0
                endY = 0
                addOnlineMove(myPlayer, startX, startY, endX, endY)
                clickCounter = 0
    ...
    private fun addOnlineMove(player: Player, startX : Int, startY : Int, endX: Int, endY: Int){
        val isWhitePlayer = player.isWhiteSide
        val startCoordinates = "${startX}${startY}"
        val endCoordinates = "${endX}${endY}"
        val data : String = "$isWhitePlayer $startCoordinates $endCoordinates"
        FirebaseDatabase.getInstance().reference.child("data").child(code).push().setValue(data)
    }
    fun openWinDialog(game : Game){
        ...
        if(game.getStatus() == GameStatus.WHITE_WIN){
            if(isCodeMaker){
                title = "You Won!"
                addMatchToDatabase("White", "You", game.getMoves(), gameName,"true")
            }
            else{
                addMatchToDatabase("White", "Your Friend", game.getMoves(), gameName,"true")
            }
        }
        if(game.getStatus() == GameStatus.BLACK_WIN){
            if(isCodeMaker){
                addMatchToDatabase("Black", "You", game.getMoves(), gameName,"true")
            }
            else{
                addMatchToDatabase("Black", "Your Friend", game.getMoves(), gameName,"true")
            }
        }
        val dialog = Dialog(this)
    ...
    private fun updateBoard(startX : Int, startY : Int, endX : Int, endY : Int){
    ...
    private fun showPosibilities ( board : Board, startX : Int, startY : Int){
        ...
        if(board.getBox(startX, startY).piece!!.isWhite!= isCodeMaker){
            return
        }
        for(i in 0..7){
            for(j in 0..7){
