...
class Queen(white : Boolean) : Piece(white) {
    ...
    private fun isNotCheck( board: Board?, start: Spot?, end: Spot?) :Boolean{
        ...
        for( i in 0..7){
            for( j in 0..7){
                if(newBoard!!.getBox(i, j).piece!=null && newBoard.getBox(i, j).piece!!.isWhite != kingSpot?.piece!!.isWhite && newBoard.getBox(i, j).piece!!.canMoveCheckVerifier(newBoard, newBoard.getBox(i, j), kingSpot)){
